...
static Real _sum_reduce(Real buffer[]) {
  ...
  while (nTotalThreads > 1) {
    int32_cuda halfPoint = ((1 + nTotalThreads) >> 1);  
...
static void _copy_from_mat_trans(Real* mat_out, const OtherReal* mat_in,
  ...
  const int32_cuda i_in = blockIdx.y * TileDim + threadIdx.y;  
  const int32_cuda j_in = blockIdx.x * TileDim + threadIdx.x;  
...
static void _copy_from_smat(Real* mat, MatrixDim mat_dim,
  ...
  if (i < mat_dim.rows) {
    ...
    for (int nz_id = nz_start + threadIdx.x; nz_id < nz_end; nz_id +=
        warpSize) {
      const int j = smat_col_idx[nz_id];  
...
static void _select_rows(const int* out_row_ptr, int* out_col_idx,
  ...
  const int out_i = blockIdx.x * blockDim.y + threadIdx.y;  
...
static void _calc_group_max_deriv(Real *deriv, const Real *vec,
  ...
  if (j < deriv_dim.rows && i < deriv_dim.cols) {
    ...
    Real vec_element = vec[vec_index],  
        max_element = maxv[maxv_index];  
...
static void _cuda_comp_obj_deriv(MatrixElement<Real> *x, int s, const Real* z,
  ...
  Real tmp_tot_objf = 0;
  int size = s / CU1DBLOCK;  
  int threshold = s - size * CU1DBLOCK;  
...
static void _group_transform_reduce(
  ...
  int j = threadIdx.y * group_size + threadIdx.x;  
...
#   pragma unroll
  ...
  const Real kSquaredNormFloor = 1.3552527156068805425e-20;  
  if (tid == 0) {
    ssum[0] = sqrt(
...
void cuda_int32_set_const(dim3 Gr, dim3 Bl, int32_cuda* mat, int32_cuda value,
                          MatrixDim d) {
{ __set_CUDAConfig(Gr, Bl); 
          
 _set_const(mat,value,d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_diag_vec_mat(alpha, mat, mat_dim, vec, mat2, mat2_row_stride,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _apply_exp_limited(mat, d, lower_limit, upper_limit);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _apply_pow_abs(mat, power, include_sign, d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_cols(dst, src, reorder, dst_dim, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_cols(dst, src, reorder, dst_dim, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_rows(dst, src, reorder, dst_dim, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_rows(alpha, dst, src, reorder, dst_dim, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _mul_rows(dst, src, reorder, dst_dim, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_rows(alpha, dst, src, dst_dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_to_rows(alpha, dst, src, reorder, src_dim, dst_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_to_rows(alpha, dst, src, src_dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _apply_ceiling(mat, ceiling_val, d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _set_diag_packed(mat,value,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_diag_packed(mat,value,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _scale_diag_packed(mat,value,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _mul_elements(mat,A,dst_d,src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _div_elements(mat,A,dst_d,src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _min(mat,other,mat_d,other_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _mul_rows_group_mat(y, x, d, src_stride, group_size);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_group_pnorm(id, iv, ov, od, id_dim, iv_stride, ov_stride,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _calc_group_max_deriv(y, x1, x2, y_dim, x1_stride, x2_stride,
      group_size);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_trans(alpha,src,dst,d,src_stride);}
          
  } else {
{ __set_CUDAConfig(Gr, Bl); 
          
 _add_mat(alpha,src,dst,d,src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_blocks_trans(alpha, src, num_row_blocks, num_col_blocks,
        dst, d, src_stride);}
          
  } else {
{ __set_CUDAConfig(Gr, Bl); 
          
 _add_mat_blocks(alpha, src, num_row_blocks, num_col_blocks, dst,
        d, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_repeated(alpha, src, src_dim, dst, dst_dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _set_mat_mat_div_mat(A,B,C,dst,d, stride_a, stride_b, stride_c);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _sy_add_tr2(alpha, beta, T, tdim, S, sdim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_vec_to_cols(alpha,col,beta,dst,d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_vec_to_rows(alpha,row,beta,dst,d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_diag_vec(alpha, mat, mat_dim, mat2, mat2_row_stride,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_mat_elements(data, srcA_data, srcB_data, dim,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _apply_mask(mat,mask,dmat,dmask);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _transform_reduce_mat_cols(result,mat,d,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _replace_value(v, dim, orig, changed);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _set_bias_params(v,a,param_1,param_2,param_3,flag,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cublas_copy_kaldi(n, x, incx, y, incy);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _vec_transform_reduce(v, value, dim, inc,
      TransReduceOp<MIN, float>());}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _trace_mat_mat_trans(A,B,dA,B_stride,value);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _trace_mat_mat<32> (A,B,dA,B_stride,value);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_diag_mat_mat_MNT(alpha,M,dim_M,N,stride_N,beta,v);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_diag_mat_mat_MTN<16> (alpha, M, stride_M, N, dim_N, beta,
                                           v, stride_v);}
          
  } else if (Bl.x == 32) {
{ __set_CUDAConfig(Gr, Bl); 
          
 _add_diag_mat_mat_MTN<32> (alpha, M, stride_M, N, dim_N, beta,
                                           v, stride_v);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_diag_mat_mat_MN<16> (alpha,M,stride_M,N,dim_N,beta,v);}
          
  } else if (Bl.x==32) {
{ __set_CUDAConfig(Gr, Bl); 
          
 _add_diag_mat_mat_MN<32>(alpha,M,stride_M,N,dim_N,beta,v);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_vec_vec(alpha,v,x,y,beta,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_matrix_add_elements(data, dim, alpha, x, num_elements);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_matrix_add_indexed_values(dim, alpha, indices, x, s, data);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_matrix_add_to_elements(alpha, mat, dim, elements);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_vector_copy_elements(data, dim, src_mat, mat_stride,
                                         transpose, elements);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_comp_obj_deriv(x,s,z,d,z2,d2,t);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _vec_copy_diag_from_packed(dst,src,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _vec_apply_floor(v,floor_val,count,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _vec_apply_ceiling(v, ceiling_val,count,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_mat_blockmat_trans(data, d, Adata, A_num_rows, A_num_cols,
        A_row_stride, A_col_stride, B_cu_data, B_num_blocks, alpha, beta);}
          
  } else {
{ __set_CUDAConfig(Gr, Bl); 
          
 _add_mat_blockmat(data, d, Adata, A_num_rows, A_num_cols,
        A_row_stride, A_col_stride, B_cu_data, B_num_blocks, alpha, beta);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _block_add_mat_mat(B_cu_data, num_blocks, C_data, C_num_cols,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _group_pnorm(y, x, d, src_stride, group_size, power);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _group_transform_reduce(y, x, d, src_stride, group_size,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_sigmoid(eout, e, y, d, e_stride, y_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_tanh(eout, e, y, d, e_stride, y_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _ensure_nonzero(x, d, epsilon, y_stride, y);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _parametric_relu(y, x, d, src_stride, a, b);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_parametric_relu(eout, e, y, d, e_stride, y_stride, a, b);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _softmax_reduce(y, x, d, src_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _log_softmax_reduce(y, x, y_dim, x_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _normalize_per_row(y, y_stride, x, x_d, target_rms, add_log_stddev);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy(y,x,copy_from,d_out,d_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _randomize(y,x,copy_from,d_out,d_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _regularize_l1(wei,grad,l1,lr,d,stride_grad);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _find_row_max_id(mat, vec_val, vec_id, d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_xent(vec_tgt,mat_net_out,vec_log_post,d);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_softmax(x, dim, value, value_stride, diff, diff_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_rows_from_vec(mat_out, d_out, v_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_log_softmax(in_deriv_dim, out_value, out_value_stride,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_col_from_mat_df(v,col,mat,dmat,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_col_from_mat_fd(v,col,mat,dmat,dim);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _sum_column_ranges(data, dim, src_data, src_dim, indices);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_row_ranges(data, dim, src_data, src_dim, indexes);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _matrix_lookup(data, dim, indices, indices_size, output);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _equal_element_mask(mat1, mat2, mask, mat1_dim, mat2_stride,
      mask_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_from_mat(mat_out,mat_in,d_out,d_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_from_mat_trans<32> (mat_out,mat_in,d_out,d_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_from_smat(mat, mat_dim, smat_row_ptr, smat_col_idx,
                              smat_val);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_from_smat_trans(mat, mat_dim, smat_row_ptr, smat_col_idx,
                                    smat_val);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _trace_mat_smat(mat, mat_dim, smat_row_ptr, smat_col_idx,
                              smat_val, trace_vec);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _trace_mat_smat_trans(mat, mat_dim, smat_row_ptr, smat_col_idx,
                                    smat_val, trace_vec);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _copy_cols_from_vec(mat_out, d_out, v_in);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _diff_normalize_per_row(id, id_stride, iv, iv_dim, od, od_stride,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _select_rows(out_row_ptr, out_col_idx, out_val, row_indexes,
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_smat(mat, mat_dim, alpha, smat_row_ptr, smat_col_idx,
                        smat_val);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _add_smat_trans(mat, mat_dim, alpha, smat_row_ptr, smat_col_idx,
                              smat_val);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _apply_exp_special(out, out_dim, in, in_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_compress_uint8_sign(src, dim, dest, dest_stride);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_compress_bounds_check(src, dim, dest, dest_stride, inv_scale);}
          
  } else {
{ __set_CUDAConfig(Gr, Bl); 
          
 _cuda_compress_no_bounds_check(src, dim, dest, dest_stride, inv_scale);}
...
{ __set_CUDAConfig(Gr, Bl); 
 ...
 _cuda_uncompress(dest, dim, src, src_stride, scale);}
